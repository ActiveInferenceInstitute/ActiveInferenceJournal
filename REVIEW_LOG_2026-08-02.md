...
## Follow-up 3 (same day) — polish

- `README.md`: added the Journal integrity workflow badge (URL returns 200).
- `docs/README.md`: Quick orientation now cross-links the root-level guides
  (`CONTRIBUTING.md`, `SECURITY.md`, `CITATION.cff`, `TO-DO.md`).
- `AGENTS.md`: added `TO-DO.md` to the entry points; wrapped the long
  "generated by" bullet.
- Channel `data/video/activeinferenceinstitute/AGENTS.md`: wrapped the
  474-char series/item description.
- Series README long lines (`Other/`, `TextbookGroup/`, `Courses/`) reviewed
  and deliberately left: they are data-dependent (long item slugs / nested
  cohort paths), matching `INDEX.md`'s established rendering.
- Re-verified: 55-file link/fence sweep clean; all three Journal-Utilities
  gates PASS; pre-existing dirty tree (5 deleted + ~154 untracked `.srt`)
  untouched.
